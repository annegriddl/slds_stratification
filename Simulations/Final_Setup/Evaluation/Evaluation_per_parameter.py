...
from utils_visualization_per_parameter import flatten_data, generate_hyperparameter_combinations_dict, descreptives, save_results_to_csv, csv_to_list, flatten_nested_lists, error_estimator, plot_combined_boxplots, grouped_bar_plot_hyperparameters
...
print('Data Loading from json file:')
...
for parameter_combination in parameter_combinatons:
    # create unique identifier of parameter_combination for saving results
    ...
    ###########################################################################################################
    ##### Evaluation of one parameter combinaiton #####
    ...
    # ['ks_statistic', 'p_value', 'intersection_area']
    ...
    error_estimator_list_name, error_estimator_values = csv_to_list(error_estimator_result, title = 'error_estimator')
    ...
    header = ['parameter_combination_string', 
            'hyperparameter_different', 'hyperparameter_same', 'hyperparameter_different_rel', #hyperparameter
    ...
    values = [parameter_combination_string, hyp_same[0], hyp_same[1], hyp_different_rel, 
            ...
            np.array(num_unique_stratified), np.array(counts_stratified), np.array(num_unique_unstratified), np.array(counts_unstratifed)]
